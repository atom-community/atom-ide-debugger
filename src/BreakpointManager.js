...
export default class BreakpointManager {
  ...
  constructor(service: IDebugService) {
    ...
    this._disposables = new UniversalDisposable(atom.workspace.observeTextEditors(this._handleTextEditor.bind(this)))
  }
  ...
  _handleTextEditor(editor: atom$TextEditor) {
    if (!this._displayControllers.has(editor)) {
      const controller = new BreakpointDisplayController(this, this._service, editor)
