...
export default class BreakpointDisplayController {
  ...
  constructor(delegate: BreakpointDisplayControllerDelegate, service: IDebugService, editor: atom$TextEditor) {
    ...
    this._disposables.addUntilDestroyed(
      ...
      Observable.merge(
        observableFromSubscribeFunction(debuggerModel.onDidChangeBreakpoints.bind(debuggerModel)),
        observableFromSubscribeFunction(this._service.viewModel.onDidChangeDebuggerFocus.bind(this._service.viewModel))
      )
   ...
   */
  _handleMarkerChange(breakpoint: IBreakpoint, event: atom$MarkerChangeEvent): void {
    const path = this._editor.getPath()
  ...
  _createBreakpointMarkerAtLine(line: number, isShadow: boolean, breakpoint: ?IBreakpoint): atom$Marker {
