...
import createPaneContainer from "@atom-ide-community/nuclide-commons-atom/create-pane-container"
...
export default class DebuggerLayoutManager {
  ...
  _overridePaneInitialHeight(dockPane: atom$Pane, newFlexScale: number, desiredHeight: number): void {
  ...
  _initializeDebuggerPanes(): void {
    ...
    this._debuggerPanes = [
      ...
      },
      {
        ...
        debuggerModeFilter: (mode: DebuggerModeType) => mode !== DebuggerMode.STOPPED,
      },
      {
        ...
        dock.dock.getPanes().forEach((pane) => {
          if (
            pane
              .getItems()
              .find(
                (item) => item instanceof DebuggerPaneViewModel || item instanceof DebuggerPaneContainerViewModel
              ) != null
          ) {
  ...
  getModelForDebuggerUri(uri: string): any {
    ...
    if (config != null) {
      return new DebuggerPaneViewModel(config, config.isLifetimeView, (pane) => this._paneDestroyed(pane))
    }
    ...
    for (const debuggerPane of this._debuggerPanes) {
      ...
      if (
        ...
        (debuggerPane.previousLocation.dock === debuggerPane.defaultLocation ||
          (debuggerPane.previousLocation.dock === debuggerPane.previousDefaultLocation &&
  ...
  debuggerModeChanged(): void {
    ...
    if (mode === DebuggerMode.STOPPING && this._previousDebuggerMode !== DebuggerMode.STOPPED) {
  ...
  _countPanesForTargetDock(dockName: string, defaultDockName: string): number {
    ...
    return this._debuggerPanes
      .filter(
        ...
        (debuggerPane) =>
          (debuggerPane.previousLocation == null || !debuggerPane.previousLocation.userHidden) &&
          (debuggerPane.debuggerModeFilter == null || debuggerPane.debuggerModeFilter(mode))
      )
      .map((debuggerPane) => {
        // Map each debugger pane to the name of the dock it will belong to.
        if (debuggerPane.previousLocation != null) {
          const previousDock = this._getWorkspaceDocks().find(
            (d) => debuggerPane.previousLocation != null && d.name === debuggerPane.previousLocation.dock
          )
  ...
  _getSavedDebuggerPaneSize(dock: { name: string, dock: atom$AbstractPaneContainer, orientation: string }): ?number {
  ...
  showDebuggerViews(): void {
    ...
    const defaultDock = this._getWorkspaceDocks().find((d) => d.name === DEBUGGER_PANELS_DEFAULT_LOCATION)
    ...
    this._debuggerPanes
      ...
      .forEach((debuggerPane) => {
        ...
        const loc =
          debuggerPane.previousLocation != null ? debuggerPane.previousLocation.dock : debuggerPane.defaultLocation
  ...
  _addPaneContainerToWorkspace(
    ...
    const containerModel = new DebuggerPaneContainerViewModel(container, dockSize)
  ...
  _getFocusedProcessMode(): DebuggerModeType {
    ...
    return viewModel.focusedProcess == null ? DebuggerMode.STOPPED : viewModel.focusedProcess.debuggerMode
  }
  ...
  hideDebuggerViews(performingLayout: boolean): void {
    ...
    atom.workspace.getPanes().forEach((pane) => {
      pane.getItems().forEach((item) => {
        if (item instanceof DebuggerPaneViewModel || item instanceof DebuggerPaneContainerViewModel) {
