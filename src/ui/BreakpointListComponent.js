...
export default class BreakpointListComponent extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    this._disposables = new UniversalDisposable(
      Observable.fromPromise(passesGK("nuclide_debugger_logging_breakpoints")).subscribe((supportsLogMessage) => {
  ...
  _computeState(): State {
    ...
    const exceptionBreakpointsCollapsed = Boolean(featureConfig.get("debugger-exceptionBreakpointsCollapsed"))
    ...
    return {
      supportsConditionalBreakpoints:
        focusedProcess != null && Boolean(focusedProcess.session.capabilities.supportsConditionalBreakpoints),
  ...
  componentDidMount(): void {
    ...
    this._disposables.add(
      ...
      observeProjectPathsAll((projectPaths) => this.setState({ activeProjects: projectPaths }))
    )
  }
  ...
  _handleBreakpointEnabledChange = (breakpoint: IBreakpoint, enabled: boolean): void => {
    this.props.service.enableOrDisableBreakpoints(enabled, breakpoint)
  }

  _handleBreakpointClick = (breakpointIndex: number, breakpoint: ?IBreakpoint): void => {
  ...
  render(): React.Node {
    const { exceptionBreakpoints, supportsConditionalBreakpoints, activeProjects, breakpoints } = this.state
    const { service } = this.props
    ...
    const breakpointGroup = (available) =>
      breakpoints
        .filter((bp) => {
          const match =
            nuclideUri.isLocal(bp.uri) || availableHosts.some((host) => this._getHostnameTranslated(bp.uri) === host)
        ...
        .map((breakpoint, i) => {
          ...
          const title =
            ...
            (available ? "" : ` - ${host}:${nuclideUri.getPath(breakpoint.uri)}`)
          ...
          const hitcountElement =
            breakpoint.hitCount != null && breakpoint.hitCount > 0 ? (
              <div className="debugger-breakpoint-hitcount">Hit count: {breakpoint.hitCount}</div>
            ) : null
          const content = (
            <div className="inline-block">
              ...
              >
                ...
                />
                <span title={title} data-path={path} data-bpid={bpId} data-line={line}>
                  <div className="debugger-breakpoint-condition-controls">
                    ...
                    <Icon
                      ...
                      data-line={line}
                      onClick={(event) => {
                        track(AnalyticsEvents.DEBUGGER_DELETE_BREAKPOINT_FROM_ICON)
    ...
    return (
      <div>
        <ListView alternateBackground={true} onSelect={this._handleBreakpointClick} selectable={true}>
          {availableBreakpoints}
        </ListView>
        {breakpoints.length === 0 ? (
          <span className="debugger-breakpoint">You currently have no source breakpoints set.</span>
        ) : null}
        {exceptionBreakpoints.length > 0 ? (
          ...
          >
            {exceptionBreakpoints.map((exceptionBreakpoint) => {
              return (
                <div className="debugger-breakpoint" key={exceptionBreakpoint.getId()}>
                  <Checkbox
                    className={classnames("debugger-breakpoint-checkbox", "debugger-exception-checkbox")}
        ...
        {unavailableBreakpoints.length > 0 ? (
          ...
          >
            <div className="debugger-unavailable-breakpoint-help">
              These breakpoints are in files that are not currently available in any project root. Add the corresponding
              local or remote project to your file tree to enable these breakpoints.
            </div>
