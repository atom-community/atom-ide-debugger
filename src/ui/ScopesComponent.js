...
export default class ScopesComponent extends React.Component<Props, State> {
  ...
  _expansionStates: Map<string /* expression */, Object /* unique reference for expression */>
  ...
  componentDidMount(): void {
    ...
    this._disposables.add(
      Observable.merge(
        observableFromSubscribeFunction(viewModel.onDidChangeDebuggerFocus.bind(viewModel)).map(() => false),
        observableFromSubscribeFunction(viewModel.onDidChangeExpressionContext.bind(viewModel)).map(() => true)
      )
        .debounceTime(100)
        .startWith(false)
  ...
  _getScopes(forceRefresh: boolean): Observable<Expected<Array<IScope>>> {
    ...
    if (focusedStackFrame == null) {
      ...
      const result = Observable.fromPromise(
        focusedStackFrame.getScopes(forceRefresh).then(
          (scopes) => Expect.value(scopes),
          (error) => Expect.error(error)
        )
      )
      return forceRefresh ? result : Observable.of(Expect.pending()).concat(result)
    }
  }
...
class ScopeComponent extends React.Component<ScopeProps> {
  render() {
    ...
    if (variables.isError) {
      return <div>Error fetching scope variables {variables.error.toString()}</div>
  ...
  _renderVariable(expression: IVariable): ?React.Element<any> {
    return (
      <div className="debugger-expression-value-row debugger-scope" key={expression.name}>
        <div className="debugger-expression-value-content">
          <ExpressionTreeComponent expression={expression} containerContext={this.props.containerContext} />
        </div>
      </div>
    )
  }
