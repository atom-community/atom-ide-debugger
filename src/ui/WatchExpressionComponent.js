...
export default class WatchExpressionComponent extends React.Component<Props, State> {
  ...
  _renderExpression = (watchExpression: IEvaluatableExpression): React.Element<any> => {
    ...
    const ExpressionComponent =
      focusedProcess == null
        ...
        : bindObservableAsProps(
            evaluateExpressionAsStream(watchExpression, focusedProcess, focusedStackFrame, "watch").map((result) => {
    ...
    return (
      <div className={classnames("debugger-expression-value-row", "debugger-watch-expression-row")} key={id}>
        <div
          className={classnames("debugger-expression-value-content", "debugger-watch-expression-value-content")}
          onDoubleClick={this._setRowBeingEdited.bind(this, id)}
        >
          {ExpressionComponent == null ? (
            <span>
              {watchExpression.name}: Not available <i>(the debugger is not running)</i>.
            </span>
          ) : (
            <ExpressionComponent />
          )}
        ...
        <div className="debugger-watch-expression-controls">
          ...
          />
          <Icon icon="x" className="debugger-watch-expression-control" onClick={this.removeExpression.bind(this, id)} />
        </div>
      </div>
    )
  }
  ...
  render(): React.Node {
    ...
    const addNewExpressionInput = (
      <AtomInput
        className={classnames("debugger-watch-expression-input", "debugger-watch-expression-add-new-input")}
