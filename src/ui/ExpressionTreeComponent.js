...
export class ExpressionTreeNode extends React.Component<ExpressionTreeNodeProps, ExpressionTreeNodeState> {
  ...
  constructor(props: ExpressionTreeNodeProps) {
    ...
    this._toggleNodeExpanded = ignoreTextSelectionEvents(this._toggleExpand.bind(this))
    this.state = {
  ...
  _fetchChildren = (): void => {
    ...
    if (this._isExpandable()) {
      this._subscription = Observable.fromPromise(this.props.expression.getChildren())
  ...
  _renderValueLine = (
    ...
    if (expression == null) {
      return <div className="nuclide-ui-expression-tree-value-container native-key-bindings">{value}</div>
    } else {
      return this.props.hideExpressionName ? (
        <div className="nuclide-ui-lazy-nested-value-container native-key-bindings">{value}</div>
      ) : (
        <div className="nuclide-ui-lazy-nested-value-container native-key-bindings">
          <span className={ValueComponentClassNames.identifier}>{expression}</span>: {value}
        </div>
      )
    }
  }
  ...
  _renderChild = (child: IExpression): React.Node => {
    ...
    return (
      <TreeItem key={nodePath}>
        <ExpressionTreeNode expression={child} expansionCache={this.props.expansionCache} nodePath={nodePath} />
      </TreeItem>
    )
  }
  ...
  _getVariableExpression(): ?IVariable {
    ...
    return (expression: any).canSetVariable == null || (expression: any).setVariable == null ? null : (expression: any)
  }
  ...
  _updateValue = (): void => {
    ...
    if (doEdit) {
      ...
      const subscription = Observable.fromPromise(variable.setVariable(pendingValue))
        .catch((error) => {
  ...
  render(): React.Node {
    ...
    if (!this.state.expanded) {
      ...
      children = this._renderValueLine(
        "Children",
        this.state.children.error != null ? this.state.children.error.toString() : NOT_AVAILABLE_MESSAGE
      )
    } else {
    ...
    return (
      <TreeList showArrows={true} className="nuclide-ui-expression-tree-value-treelist">
        <NestedTreeItem
