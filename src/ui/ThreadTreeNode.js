...
export default class ThreadTreeNode extends React.Component<Props, State> {
  ...
  componentDidMount(): void {
    ...
    const changedCallStack = observableFromSubscribeFunction(model.onDidChangeCallStack.bind(model))
    ...
    this._disposables.add(
      Observable.merge(observableFromSubscribeFunction(viewModel.onDidChangeDebuggerFocus.bind(viewModel))).subscribe(
        () => {
      ...
      additionalFocusedCheck
        ...
        .switchMap(() => {
          ...
          const newIsCollapsed = this.state.isCollapsed && !this._threadIsFocused()
          ...
          return this._getFrames(newIsCollapsed ? 1 : this.state.callStackLevels).switchMap((frames) =>
            Observable.of({
              frames,
              newIsCollapsed,
            })
  ...
  _handleStackFrameClick = (clickedRow: { frame: IStackFrame }, callFrameIndex: number): void => {
    ...
    return (
      <div
        className={classnames({
          "debugger-container-new-disabled": this.props.thread.process.debuggerMode === DebuggerMode.RUNNING,
        })}
      >
  ...
  render(): React.Node {
    ...
    const canTerminateThread =
      Boolean(thread.process.session.capabilities.supportsTerminateThreadsRequest) &&
      thread.threadId > 0 &&
      thread.stopped
    ...
    const formattedTitle = (
      <span
        ...
        className={isFocused ? classnames("debugger-tree-process-thread-selected") : ""}
    ...
    if (!thread.stopped || (!stackFrames.isPending && !stackFrames.isError && stackFrames.value.length === 0)) {
      return <TreeItem className="debugger-tree-no-frames">{formattedTitle}</TreeItem>
    }

    const LOADING = (
      <div className={classnames("debugger-expression-value-row", "debugger-tree-no-frames")}>
