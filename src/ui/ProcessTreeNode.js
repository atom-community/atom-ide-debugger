...
export default class ProcessTreeNode extends React.Component<Props, State> {
  ...
  componentDidMount(): void {
    ...
    this._disposables.add(
      Observable.merge(observableFromSubscribeFunction(viewModel.onDidChangeDebuggerFocus.bind(viewModel)))
        .let(fastDebounce(15))
      ...
      observableFromSubscribeFunction(service.onDidChangeProcessMode.bind(service)).subscribe(() =>
    ...
    if (filter == null) {
      ...
      return (
        ...
        thread
          ...
          .some(
            (frame) =>
              frame.name.match(filterRegEx) || (frame.source.name != null && frame.source.name.match(filterRegEx))
          )
      )
    }
  }
  ...
  render() {
    ...
    const readOnly =
      service.viewModel.focusedProcess != null && service.viewModel.focusedProcess.configuration.isReadOnly
    ...
    const fileIcon = this.state.pendingStart ? (
      <div className="inline-block" title="Starting debugger...">
        <LoadingSpinner size={LoadingSpinnerSizes.EXTRA_SMALL} className="inline-block" />
      </div>
    ) : (
      <span
    ...
    const formattedTitle = (
      <span>
        ...
        <span
          onClick={handleTitleClick}
          className={
            isFocused ? "debugger-tree-process debugger-tree-process-thread-selected" : "debugger-tree-process"
          }
          title={title}
        >
          {title}
    ...
    const selectedThreadFiltered =
      threads.some((t) => t === focusedThread) && !filteredThreads.some((t) => t === focusedThread)
    ...
    return threads.length === 0 ? (
      ...
      <NestedTreeItem title={formattedTitle} collapsed={isCollapsed} onSelect={this.handleSelect}>
        {filteredThreads.length === 0 && threads.length > 0 ? (
          selectedThreadFiltered ? (
            focusedThreadHiddenWarning
          ) : (
            <span className="debugger-thread-no-match-text">No threads match the current filter.</span>
          )
        ) : (
          filteredThreads
