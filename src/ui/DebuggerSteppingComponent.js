...
import { LoadingSpinner, LoadingSpinnerSizes } from "@atom-ide-community/nuclide-commons-ui/LoadingSpinner"
...
const STEP_OUT_ICON = (
  <svg viewBox="0 0 100 100">
    <circle cx="50" cy="75" r="10" />
    <polygon points="42,20 57,20 57,40 72,40 50,60 28,40 42,40" transform="rotate(180, 50, 40)" />
  </svg>
)
...
> {
    ...
    this._disposables.add(
      Observable.merge(
        ...
        .subscribe(() => {
          ...
          const debuggerMode = focusedProcess == null ? DebuggerMode.STOPPED : focusedProcess.debuggerMode

          this.setState((prevState) => ({
            focusedProcess,
            focusedThread,
            waitingForPause: prevState.waitingForPause && debuggerMode === DebuggerMode.RUNNING,
          }))
        })
    )
  }
  ...
  render(): React.Node {
    ...
    const loadingIndicator = !isPausing ? null : (
      <LoadingSpinner className="debugger-stepping-playpause-button-loading" size={LoadingSpinnerSizes.EXTRA_SMALL} />
    )
