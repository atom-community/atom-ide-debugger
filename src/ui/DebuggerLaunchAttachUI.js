...
import type { DebuggerConfigAction, DebuggerLaunchAttachProvider } from "@atom-ide-community/nuclide-debugger-common"
...
function setLastUsedDebugger(host: string, action: DebuggerConfigAction, debuggerDisplayName: string): void {
  const key = "DEBUGGER_LAST_USED_" + host + "_" + action
  localStorage.setItem(key, debuggerDisplayName)
}

function getLastUsedDebugger(host: string, action: DebuggerConfigAction): ?string {
  const key = "DEBUGGER_LAST_USED_" + host + "_" + action
...
export default class DebuggerLaunchAttachUI extends React.Component<Props, State> {
  props: Props
  state: State
  ...
  _rememberTab(): void {
    ...
    // button is clicked.
    const host = nuclideUri.isRemote(this.props.connection) ? nuclideUri.getHostname(this.props.connection) : "local"
  ...
  UNSAFE_componentWillReceiveProps(nextProps: Props) {
    const host = nuclideUri.isRemote(nextProps.connection) ? nuclideUri.getHostname(nextProps.connection) : "local"
  ...
  async _getProviderIfEnabled(provider: DebuggerLaunchAttachProvider): Promise<?DebuggerLaunchAttachProvider> {
  ...
  setState(partialState: $Shape<State> | ((State, Props) => $Shape<State> | void), callback?: () => mixed): void {
  ...
  render(): React.Node {
    ...
    if (tabs.length > 0) {
      let selectedTab =
        this.state.selectedProviderTab != null ? this.state.selectedProviderTab : this.state.enabledProviders[0].tabName
      ...
      const debuggerConfigPage = provider.provider
        ...
        .getComponent(selectedTab, (valid) => this._setConfigValid(valid), defaultConfig)
      ...
      providerContent = (
        <div>
          ...
          />
          <div className="debugger-launch-attach-tabcontent">{debuggerConfigPage}</div>
        </div>
      )
    } else {
      ...
      providerContent = (
        <div className="debugger-launch-attach-tabcontent">
          ...
          <a href="https://atom.io/packages/search?q=atom-ide-debugger-">atom.io/packages</a>
        </div>
      )
    }
    ...
    return (
      <div className="padded debugger-launch-attach-container">
        ...
        <div className="debugger-launch-attach-actions">
          <ButtonGroup>
            <Button onClick={() => atom.commands.dispatch(atom.views.getView(atom.workspace), "core:cancel")}>
              Cancel
            </Button>
            <Button
