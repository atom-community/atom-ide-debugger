import type { IExpression, IEvaluatableExpression, IProcess, IStackFrame, ContextType } from "./types"
...
function getEditorLineNumber(editor: atom$TextEditor, target: HTMLElement): ?number {
  let node = target
  while (node != null) {
...
export async function openSourceLocation(path: string, line: number): Promise<?atom$TextEditor> {
...
export function evaluateExpressionAsStream(
  ...
  return Observable.fromPromise(expression.evaluate(focusedProcess, focusedStackFrame, context))
