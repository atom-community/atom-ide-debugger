  ...
  _disposables = new UniversalDisposable(
    ...
    atom.commands.add("atom-workspace", {
      "debugger:show-attach-dialog": (event) => {
        const selectedTabName: any = idx(event, (_) => _.detail.selectedTabName)
          ...
          submenu: [
            ...
            },
            {
              ...
              shouldDisplay: (event) =>
                _executeWithEditorPath(
                  event,
                  (filePath, line) => _service.getModel().getBreakpointAtLine(filePath, line) != null
                ) || false,
            },
            {
              label: "Edit Breakpoint...",
            ...
            },
            {
              ...
              shouldDisplay: (event) => {
                ...
                if (_service.getDebuggerMode() === DebuggerMode.STOPPED || textEditor == null) {
                  return false
                }
            ...
            },
            {
              ...
              shouldDisplay: (event) => _service.getDebuggerMode() === DebuggerMode.PAUSED && !_isReadOnlyTarget(),
            },
          ],
        },
        { type: "separator" },
...
function _setProvidersForConnection(connection: NuclideUri): void {
  const key = nuclideUri.isRemote(connection) ? nuclideUri.getHostname(connection) : "local"
...
async function _getSuggestions(request: atom$AutocompleteRequest): Promise<?Array<atom$AutocompleteSuggestion>> {
...
function _registerCommandsContextMenuAndOpener(): UniversalDisposable {
  const disposable = new UniversalDisposable(
    ...
    _service.onDidChangeProcessMode(() => _layoutManager.debuggerModeChanged()),
    _service.viewModel.onDidChangeDebuggerFocus(() => _layoutManager.debuggerModeChanged()),
    atom.commands.add("atom-workspace", {
...
function _executeWithEditorPath<T>(event: any, fn: (filePath: string, line: number) => T): ?T {
...
function _renderConfigDialog(panel: atom$Panel, args: LaunchAttachDialogArgs, dialogCloser: () => void): void {
  ...
  const options = _uiModel
    .getConnections()
    .map((connection) => {
      const displayName = nuclideUri.isRemote(connection) ? nuclideUri.getHostname(connection) : "localhost"
      return {
        value: connection,
        label: displayName,
      }
  ...
  ReactDOM.render(
    <DebuggerLaunchAttachUI
      ...
      connectionChanged={(newValue: ?string) => {
        ...
        _renderConfigDialog(panel, { dialogMode: args.dialogMode }, dialogCloser)
      }}
...
function _addToWatch() {
  ...
  const selectedText = editor.getTextInBufferRange(trimRange(editor, editor.getSelectedBufferRange()))
    ...
    focusedThread
      ...
      .subscribe((expectedStack) => {
        expectedStack.getOrDefault([]).forEach((item, i) => {
          ...
          callstackText += `${i}\t${item.name}\t${path}:${item.range.start.row}${os.EOL}`
        })
  ...
  const updateSelectedConnection = (directory) => {
    ...
    if (_selectedDebugConnection != null) {
      ...
      if (nuclideUri.isRemote(conn)) {
        ...
        _selectedDebugConnection = nuclideUri.createRemoteUri(nuclideUri.getHostname(conn), "/")
      } else {
...
export function consumeDebuggerConfigurationProviders(providers: Array<DebuggerConfigurationProvider>): IDisposable {
  invariant(Array.isArray(providers))
  const disposable = new UniversalDisposable()
  providers.forEach((provider) => disposable.add(addDebugConfigurationProvider(provider)))
  return disposable
}
...
export function consumeNotifications(
  raiseNativeNotification: (title: string, body: string, timeout: number, raiseIfAtomHasFocus: boolean) => ?IDisposable
): void {
...
export function consumeDatatipService(service: DatatipService): IDisposable {
  const disposable = new UniversalDisposable(service.addProvider(_createDatatipProvider()), setDatatipService(service))
  _disposables.add(disposable)
  return disposable
}
...
function createDebuggerView(model: mixed): ?HTMLElement {
  let view = null
  if (model instanceof DebuggerPaneViewModel || model instanceof DebuggerPaneContainerViewModel) {
